"""Main FastAPI application with optimized lazy loading for Lambda."""
...
# LAZY LOADING: Global variables for singletons
...
def get_memory_manager():
    """Lazy-load memory manager singleton."""
    global _memory_manager, _initialization_in_progress
    ...
    with _init_lock:
        if _memory_manager is None:
            ...
            logger.info("Initializing MemoryManager (lazy load)...")
            try:
...
def get_agent():
    ...
    with _init_lock:
        if _agent is None:
            logger.info("Initializing ReActAgent (lazy load)...")
            from app.agent.agent import ReActAgent
            memory_manager = get_memory_manager()
            if memory_manager:
                _agent = ReActAgent(memory_manager=memory_manager)
                logger.info("ReActAgent initialized successfully")
            else:
                raise RuntimeError("Cannot initialize agent without memory manager")

    return _agent


# Health check endpoint - DOES NOT trigger initialization
...
# Chat endpoint - triggers lazy initialization only when needed
...
async def chat(request: ChatRequest):
    ...
    """
    try:
        # Lazy load resources only when actually needed
        memory_manager = get_memory_manager()
        agent = get_agent()
